...
@ccclass("ModalMgr")
export class ModalMgr extends cc.Component {
     ...
     * @param isOpen 
     */
    public checkModalWinAndPopUp(coms: UIBase[], maskArr: { [key: string]: cc.Node }, isOpen: boolean = true) {
        //保证永远只显示一个遮罩 显示
                ...
                if (maskArr[maskKey]) {
                    if (tcom.modalType.opacity >= ModalOpacity.OpacityZero && !hasShow) {
                        ...
                        if (isOpen) {
                            ...
                            cc.Tween.stopAllByTarget(maskArr[maskKey]);
                            ...
                            //     }
                            // }).union().start();
                            cc.tween(maskArr[maskKey])
                                .to(t, { opacity: opac},{ easing: 'linear'})
                                // 当前面的动作都执行完毕后才会调用这个回调函数
                                .call(() => { 
                                    if (tcom!.modalType.clickMaskClose) {
                                        maskArr[maskKey].off(cc.Node.EventType.TOUCH_START);
                                        maskArr[maskKey].on(cc.Node.EventType.TOUCH_START, () => {
                                            UIManager.getInstance().closeForm(tcom!.fid);
                                        });
                                        
                                        if(tcom.getComponent(cc.BlockInputEvents) == null){
                                            tcom.addComponent(cc.BlockInputEvents);
                                        }
                                    }
                                })
                                .start()
